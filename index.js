// FALL INTO CODINF CHALLENGE
//w1 - Registration (beginner)
...
/////////////////////////////////////////////
/*
Today is the first day of a new school year! Last year's freshmen are becoming sophomores, last year's juniors are becoming seniors! But wait, there's something wrong with the school's records. Looks like there was some kind of bug in the system. Last year's freshmen are still marked as "freshmen." They never got promoted to sophomores! This can cause all kinds of problems.

Before we can add the new incoming class of freshmen, we have to go through the list of all of our students, find the students still marked as freshmen, and create a new list of these students with their status updated to "sophomores." Actually, it sounds like we can extract this logic to a generic function and make it reusable for a lot of different cases.
...
function getAttendance(enrolledStudents){
  ...
  enrolledStudents.forEach(currentStudent => {
    attendanceCount[currentStudent] = 0;
    attendanceInfo[currentStudent] = '0%';
  })
  attendanceCount.totalClasses = 0;
  
  return (...names) => {
    const allNames = [...names];
    for(let i = 0; i < allNames.length; i++) {
      if(!enrolledStudents.includes(allNames[i])) {
        return 'there are students in class that are not enrolled';
      }
    }
    attendanceCount.totalClasses++;
    let attendancePercentage = 0;
    for(let i = 0; i < allNames.length; i++) {
       if(attendanceCount[allNames[i]] >= 0) {
         attendanceCount[allNames[i]]++;
       }
    }
    for(let i = 0; i < enrolledStudents.length; i++) {
      attendanceInfo[enrolledStudents[i]] = Math.floor(attendanceCount[enrolledStudents[i]] / attendanceCount.totalClasses * 100) + '%';
      attendancePercentage += Math.floor(attendanceCount[enrolledStudents[i]] / attendanceCount.totalClasses * 100);
    }
    attendanceInfo.attendance = Math.floor(attendancePercentage/enrolledStudents.length) + '%';
    return attendanceInfo;
  }
}
//Uncomment these to test your code!
const studentsList = ['victoria','rachel','max'];
let studentAttendance = getAttendance(studentsList);

console.log(studentAttendance('max', 'rachel')); 
// should log --> { max: '100%', rachel: '100%', victoria: '0%', attendance: '66%' }
console.log(studentAttendance('victoria','rachel'));
...
Student.prototype.printCourses = function(){
  return this.courses.map(course => {
    return(`${course.courseName} taught by ${course.teacher.name}`)
  })
}